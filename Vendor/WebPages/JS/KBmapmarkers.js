...
function Map (name) {
  ...
  // changes moran dadon added new parameter
  this.addMarker = function (icon, cordX, cordY, name, markerUrl, address) {
    var markerName
    ...
    this.mapMarkers[markerName] = new MapMarker(markerName, icon, cordX, cordY, this, markerUrl, address)
  }
  ...
  this.importJSON = function (mapDataJSON) {
    ...
    for (importedMarkerName in this.mapDataJSON) {
      // add new marker
      // changes moran dadon added another param
      this.addMarker(this.mapDataJSON[importedMarkerName].icon, this.mapDataJSON[importedMarkerName].cordX, this.mapDataJSON[importedMarkerName].cordY, importedMarkerName, this.mapDataJSON[importedMarkerName].markerUrl, this.mapDataJSON[importedMarkerName].address)
...
function MapMarker (name, icon, cordX, cordY, map, markerUrl, address) {
  this.map = map
  this.name = name
  this.icon = icon
  this.cordX = cordX
...
jQuery(document).ready(function () {
  ...
  jQuery(document).on('markerClick', function (event, mapMarker) {
    ...
    console.log('markerClick: ' + mapMarker.markerUrl)
    if (mapMarker.markerUrl != '') { window.open(mapMarker.markerUrl) }
  })
