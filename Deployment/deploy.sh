    ...
    while IFS= read -r line
	   ...
	   #get lightning configuration
           if [ "$IOT" = "1" ]; then
               ...
               ssh -n $targetVendor "~/lightning/lightningd/lightningd --network=$bitcoinNetwork --log-level=debug --log-file=lightningProcessLog.log --addr=0.0.0.0:9735 --bitcoin-rpcuser=ubuntu --bitcoin-rpcpassword=ubuntu --daemon &"
               echo "Start lightning"
               #ssh -n $target "mkdir -p ~/.lightning"
               #scp -r $lightningSource $lightningTarget
               ssh -n $target "~/lightning/lightningd/lightningd --network=$bitcoinNetwork --log-level=debug --log-file=lightningProcessLog.log --addr=0.0.0.0:9735 --bitcoin-rpcuser=ubuntu --bitcoin-rpcpassword=ubuntu --daemon &"
    ...
    if [ "$ALL" = "1" ]; then
        ...
        while IFS= read -r line
             do
               ...
               ssh -n $target "if pgrep -x lightningd > /dev/null; then cd ~/patching-lightning/Distributor/ && pkill node ; else pkill node ; nohup ~/lightning/lightningd/lightningd --network=$bitcoinNetwork --log-level=debug --log-file=lightningProcessLog.log --addr=0.0.0.0:9735 --bitcoin-rpcuser=ubuntu --bitcoin-rpcpassword=ubuntu --daemon & fi"
        ...
        while IFS= read -r line
              do
               ...
               ssh -n $target "pkill node"
               ssh -n $target "pkill lightning"
               ssh -n $target "if pgrep -x lightningd > /dev/null; then cd ~/patching-lightning/IoT/ && pkill node ; else nohup ~/lightning/lightningd/lightningd --network=$bitcoinNetwork --log-level=debug --log-file=lightningProcessLog.log --addr=0.0.0.0:9735 --bitcoin-rpcuser=ubuntu --bitcoin-rpcpassword=ubuntu --daemon & fi"
        ...
        while IFS= read -r line
             do
               ...
               ssh -n $target "pkill node"
               ssh -n $target "pkill lightning"
               ssh -n $target "if pgrep -x lightningd > /dev/null; then cd ~/patching-lightning/Distributor/ && pkill node ; else nohup ~/lightning/lightningd/lightningd --network=$bitcoinNetwork --log-level=debug --log-file=lightningProcessLog.log --addr=0.0.0.0:9735 --bitcoin-rpcuser=ubuntu --bitcoin-rpcpassword=ubuntu --daemon & fi"
